...
export const getRecipesPaginated = (db: Db, offset: number, limit: number) =>
  db.select().from(recipesTable).limit(limit).offset(offset).all();

export const getRecipesCount = (db: Db) =>
  db
    .select({ count: sql<number>`count(*)` })
    .from(recipesTable)
    .get()!.count;

export const searchRecipesPaginated = (
  db: Db,
  query: string,
  offset: number,
  limit: number,
...
export const searchRecipesCount = (db: Db, query: string) => {
  const pattern = `%${query}%`;
  return db
    .select({ count: sql<number>`count(*)` })
    .from(recipesTable)
...
export const getRecipesByTagsPaginated = (
  db: Db,
  tagIds: string[],
  offset: number,
  limit: number,
  ...
  const subquery = db
    .select({ recipeId: recipeTagsTable.recipeId })
    .from(recipeTagsTable)
    .where(inArray(recipeTagsTable.tagId, tagIds))
    .groupBy(recipeTagsTable.recipeId)
    .having(sql`count(*) = ${tagIds.length}`);

  return db
    .select()
    .from(recipesTable)
    .where(inArray(recipesTable.id, subquery))
    .limit(limit)
    .offset(offset)
    .all();
};

export const getRecipesByTagsCount = (db: Db, tagIds: string[]) => {
  const subquery = db
    .select({ recipeId: recipeTagsTable.recipeId })
    .from(recipeTagsTable)
    .where(inArray(recipeTagsTable.tagId, tagIds))
