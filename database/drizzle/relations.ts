...
export const relations = defineRelations(schema, (r) => ({
  ...
  // ── 搭配 (combos) ──
  combosTable: {
    comboRecipes: r.many.comboRecipesTable({
      from: r.combosTable.id,
      to: r.comboRecipesTable.comboId,
    }),
    recipes: r.many.recipesTable({
      from: r.combosTable.id.through(r.comboRecipesTable.comboId),
      to: r.recipesTable.id.through(r.comboRecipesTable.recipeId),
    }),
  },

  // ── 搭配-食谱关联 (junction) ──
  comboRecipesTable: {
    combo: r.one.combosTable({
      from: r.comboRecipesTable.comboId,
      to: r.combosTable.id,
    }),
    recipe: r.one.recipesTable({
      from: r.comboRecipesTable.recipeId,
      to: r.recipesTable.id,
    }),
  },
}));
