...
describe("Recipe Queries", () => {
  ...
  it("should create step with optional name", () => {
    const recipe = recipeQueries.insertRecipe(db, { name: "Test Recipe" });
    const steps = recipeQueries.upsertSteps(db, recipe.id, [
      { content: "Step content", sortOrder: 0, name: "准备阶段" },
      { content: "Step 2", sortOrder: 1 },
    ]);
    expect(steps).toHaveLength(2);
    expect(steps[0].name).toBe("准备阶段");
    expect(steps[1].name).toBeNull();
  });
...
describe("Recipe Relations", () => {
  ...
  it("should associate ingredients with recipes", () => {
    ...
    recipeQueries.upsertRecipeIngredients(db, recipe.id, [
      { ingredientId: ing.id, quantity: "500", unit: "g", note: "带皮" },
    ]);
